...
if [ "$HOST_PLATFORM" = "Win64" ]; then
  ./Engine/Build/BatchFiles/RunUAT.bat Turnkey -command=VerifySdk -platform=$TARGET_PLATFORM -UpdateIfNeeded -project="$TEMPO_ROOT/Tempo.uproject" BuildCookRun -nop4 -utf8output -nocompileeditor \
  ...
  ./Engine/Build/BatchFiles/RunUAT.sh Turnkey -command=VerifySdk -platform=$TARGET_PLATFORM -UpdateIfNeeded -project="$TEMPO_ROOT/Tempo.uproject" BuildCookRun -nop4 -utf8output -nocompileeditor \
