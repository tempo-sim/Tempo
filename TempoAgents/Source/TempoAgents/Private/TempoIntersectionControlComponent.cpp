...
void UTempoIntersectionControlComponent::SetupTrafficControllers()
{
	...
	if (World == nullptr)
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - SetupTrafficControllers - World is invalid."));
		return;
	}
...
void UTempoIntersectionControlComponent::SetupTrafficControllerMeshData()
{
	...
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - SetupTrafficControllerMeshData - OwnerActor is invalid."));
		return;
	}

	if (!OwnerActor->Implements<UTempoIntersectionInterface>())
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - SetupTrafficControllerMeshData - OwnerActor does not implement UTempoIntersectionInterface."));
		return;
	}
	...
	if (World == nullptr)
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - SetupTrafficControllerMeshData - World is invalid."));
		return;
	}
	...
	for (int32 ConnectionIndex = 0; ConnectionIndex < NumConnections; ++ConnectionIndex)
	{
		...
		{
			UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - SetupTrafficControllerMeshData - Could not create valid MeshComponent with name: %s."), *MeshComponentName.ToString());
			return;
		}
...
void UTempoIntersectionControlComponent::SetupTrafficControllerRuntimeData()
{
	...
	if (OwnerActor == nullptr)
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - SetupTrafficControllerRuntimeData - OwnerActor is invalid."));
		return;
	}

	if (!OwnerActor->Implements<UTempoIntersectionInterface>())
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - SetupTrafficControllerRuntimeData - OwnerActor does not implement UTempoIntersectionInterface."));
		return;
	}
	...
	if (World == nullptr)
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - SetupTrafficControllerRuntimeData - World is invalid."));
		return;
	}
	...
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - SetupTrafficControllerRuntimeData - Failed to get TrafficControllerRegistrySubsystem."));
		return;
	}
...
void UTempoIntersectionControlComponent::DestroyTrafficControllerMeshData()
{
	...
	if (OwnerActor == nullptr)
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - DestroyTrafficControllers - OwnerActor is invalid."));
		return;
	}
...
bool UTempoIntersectionControlComponent::TryGetIntersectionTrafficControllerLocation(const AActor* IntersectionQueryActor, int32 ConnectionIndex, ETempoTrafficControllerType TrafficControllerType, const FTempoTrafficControllerMeshInfo& TrafficControllerMeshInfo, ETempoCoordinateSpace CoordinateSpace, FVector& OutIntersectionTrafficControllerLocation) const
{
	...
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - TryGetIntersectionTrafficControllerLocation - IntersectionQueryActor is invalid."));
		return false;
	}
	...
	if (TrafficControllerType == ETempoTrafficControllerType::TrafficLight)
	{
		...
		{
			UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - Failed to get Traffic Light Anchor when determining Traffic Light Location - Failed to get Connected Road Actor for Actor: %s at ConnectionIndex: %d."), *IntersectionQueryActor->GetName(), TrafficLightAnchorConnectionIndex);
			return false;
		}
	...
	else
	{
		...
		{
			UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - Failed to get Connected Road Actor when determining Stop Sign Location - Failed to get Connected Road Actor for Actor: %s at ConnectionIndex: %d."), *IntersectionQueryActor->GetName(), ConnectionIndex);
			return false;
		}
...
bool UTempoIntersectionControlComponent::TryGetIntersectionTrafficControllerRotation(const AActor* IntersectionQueryActor, int32 ConnectionIndex, ETempoTrafficControllerType TrafficControllerType, const FTempoTrafficControllerMeshInfo& TrafficControllerMeshInfo, ETempoCoordinateSpace CoordinateSpace, FRotator& OutIntersectionTrafficControllerRotation) const
{
	if (IntersectionQueryActor == nullptr)
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - TryGetIntersectionTrafficControllerRotation - IntersectionQueryActor is invalid."));
		return false;
	}
...
bool UTempoIntersectionControlComponent::TryGetRoadConfigurationInfoForIntersection(const AActor* IntersectionQueryActor, int32 SourceConnectionIndex, ETempoRoadConfigurationDescriptor RoadConfigurationDescriptor, TArray<FTempoRoadConfigurationInfo>& OutRoadConfigurationInfos) const
{
	if (IntersectionQueryActor == nullptr)
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - TryGetRoadConfigurationInfoForIntersection - IntersectionQueryActor is invalid."));
		return false;
	}
	...
	else
	{
		...
		{
			UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - TryGetRoadConfigurationInfoForIntersection - Can't access TempoAgentsSettings.  Using hard-coded default settings."));
		}
...
bool UTempoIntersectionControlComponent::TryGetStraightestConnectionIndexForIntersection(const AActor* IntersectionQueryActor, int32 SourceConnectionIndex, int32& OutDestConnectionIndex, float& OutConnectionDotProduct) const
{
	if (IntersectionQueryActor == nullptr)
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - GetStraightestConnectionIndexForIntersection - IntersectionQueryActor is invalid."));
		return false;
	}
	...
	if (TempoAgentsSettings == nullptr)
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - GetStraightestConnectionIndexForIntersection - Can't access TempoAgentsSettings.  Using hard-coded default settings."));
	}
...
void UTempoIntersectionControlComponent::SetupTrafficLightRuntimeData(AActor& OwnerActor, UMassTrafficControllerRegistrySubsystem& TrafficControllerRegistrySubsystem)
{
	...
	for (int32 ConnectionIndex = 0; ConnectionIndex < NumConnections; ++ConnectionIndex)
	{
		...
		{
			UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - SetupTrafficLightRuntimeData - Failed to get Connected Road Actor for Actor: %s at ConnectionIndex: %d."), *OwnerActor.GetName(), ConnectionIndex);
			return;
		}
...
AActor* UTempoIntersectionControlComponent::GetConnectedRoadActor(const AActor& IntersectionQueryActor, int32 ConnectionIndex) const
{
	...
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - Failed to get Connected Road Actor for Actor: %s at ConnectionIndex: %d."), *IntersectionQueryActor.GetName(), ConnectionIndex);
		return nullptr;
	}

	if (!RoadActor->Implements<UTempoRoadInterface>())
	{
		UE_LOG(LogTempoAgents, Error, TEXT("UTempoIntersectionControlComponent - Connected Road Actor for Actor: %s at ConnectionIndex: %d doesn't implement UTempoRoadInterface."), *IntersectionQueryActor.GetName(), ConnectionIndex);
		return nullptr;
	}
