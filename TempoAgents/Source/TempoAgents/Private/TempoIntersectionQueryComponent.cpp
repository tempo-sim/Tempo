...
bool UTempoIntersectionQueryComponent::TryGetIntersectionEntranceControlPointIndex(const AActor* IntersectionQueryActor, int32 ConnectionIndex, int32& OutIntersectionEntranceControlPointIndex) const
{
	...
	{
		UE_LOG(LogTempoAgents, Error, TEXT("Tempo Intersection Query Component - Failed to get Intersection Entrance Control Point Index for Actor: %s at ConnectionIndex: %d."), *IntersectionQueryActor->GetName(), ConnectionIndex);
		return false;
	}
																 ...
																 const AActor* DestConnectionActor, const TArray<FTempoLaneConnectionInfo>& DestLaneConnectionInfos, const int32 DestLaneConnectionQueryIndex, const TArray<FLaneConnectionCandidate>& AllCandidates) const
{
	if (!IsConnectedRoadActor(SourceConnectionActor))
	{
		...
		UE_LOG(LogTempoAgents, Error, TEXT("Lane Filtering - ShouldFilterLaneConnection - SourceConnectionActor (%s) is not a connected Road Actor of %s."), *SourceConnectionActorName, *GetOwner()->GetName());
		return false;
	}
	...
	if (!IsConnectedRoadActor(DestConnectionActor))
	{
		...
		UE_LOG(LogTempoAgents, Error, TEXT("Lane Filtering - ShouldFilterLaneConnection - DestConnectionActor (%s) is not a connected Road Actor of %s."), *DestConnectionActorName, *GetOwner()->GetName());
		return false;
	}
	...
	{
		UE_LOG(LogTempoAgents, Error, TEXT("Lane Filtering - ShouldFilterLaneConnection - Connection tag filter not found in any tag group for SourceConnectionActor (%s)."), *SourceConnectionActor->GetName());
		return false;
	}

	if (SourceMatchingTagGroups.Num() > 1)
	{
		UE_LOG(LogTempoAgents, Error, TEXT("Lane Filtering - ShouldFilterLaneConnection - Connection tag filter found in more than one tag group for SourceConnectionActor (%s)."), *SourceConnectionActor->GetName());
		return false;
	}
	...
	{
		UE_LOG(LogTempoAgents, Error, TEXT("Lane Filtering - ShouldFilterLaneConnection - Could not get min lane index in lane connections for SourceConnectionActor (%s)."), *SourceConnectionActor->GetName());
		return false;
	}
	...
	{
		UE_LOG(LogTempoAgents, Error, TEXT("Lane Filtering - ShouldFilterLaneConnection - Could not get max lane index in lane connections for SourceConnectionActor (%s)."), *SourceConnectionActor->GetName());
		return false;
	}
	...
	{
		UE_LOG(LogTempoAgents, Error, TEXT("Lane Filtering - ShouldFilterLaneConnection - Could not find connection candidate in all candidates for SourceConnectionActor (%s)."), *SourceConnectionActor->GetName());
		return false;
	}
	...
	switch (ThisCandidate.TurnType)
	{
		...
		case EZoneGraphTurnType::Right:
		{
			...
			{
				UE_LOG(LogTempoAgents, Error, TEXT("DestLaneIndices empty for right turn"));
				return false;
			}
...
bool UTempoIntersectionQueryComponent::TryGetCrosswalkIntersectionConnectorInfoEntry(const AActor* IntersectionQueryActor, int32 CrosswalkIntersectionIndex, int32 CrosswalkIntersectionConnectionIndex, const TMap<int32, FCrosswalkIntersectionConnectorInfo>& CrosswalkIntersectionConnectorInfoMap, AActor*& OutCrosswalkRoadModule, float &OutCrosswalkIntersectionConnectorDistance) const
{
	...
	const auto& TryGetCrosswalkIntersectionEntranceOuterDistance = [this, &IntersectionQueryActor](const AActor& CrosswalkRoadModule, int32 CrosswalkIntersectionIndex, float &OutCrosswalkIntersectionEntranceOuterDistance)
	{
		...
		const AActor* RoadQueryActor = UTempoCoreUtils::CallBlueprintFunction(IntersectionQueryActor, ITempoIntersectionInterface::Execute_GetConnectedTempoRoadActor, ConnectionIndex);
		if (!ensureMsgf(RoadQueryActor != nullptr, TEXT("Couldn't get valid Connected Road Actor for Actor: %s at ConnectionIndex: %d in UTempoIntersectionQueryComponent::TryGetCrosswalkIntersectionConnectorInfoEntry."), *IntersectionQueryActor->GetName(), ConnectionIndex))
		{
			return false;
		}
...
bool UTempoIntersectionQueryComponent::TryGetNearestRoadControlPointIndex(const AActor& IntersectionQueryActor, int32 ConnectionIndex, int32& OutNearestRoadControlPointIndex) const
{
	...
	{
		UE_LOG(LogTempoAgents, Error, TEXT("Tempo Intersection Query Component - Failed to get Nearest Road Control Point Index for Connected Road Actor for Actor: %s at ConnectionIndex: %d."), *IntersectionQueryActor.GetName(), ConnectionIndex);
		return false;
	}
...
AActor* UTempoIntersectionQueryComponent::GetConnectedRoadActor(const AActor& IntersectionQueryActor, int32 ConnectionIndex) const
{
	...
	if (RoadActor == nullptr)
	{
		UE_LOG(LogTempoAgents, Error, TEXT("Tempo Intersection Query Component - Failed to get Connected Road Actor for Actor: %s at ConnectionIndex: %d."), *IntersectionQueryActor.GetName(), ConnectionIndex);
		return nullptr;
	}

	if (!RoadActor->Implements<UTempoRoadInterface>())
	{
		UE_LOG(LogTempoAgents, Error, TEXT("Tempo Intersection Query Component - Connected Road Actor for Actor: %s at ConnectionIndex: %d doesn't implement UTempoRoadInterface."), *IntersectionQueryActor.GetName(), ConnectionIndex);
		return nullptr;
	}
