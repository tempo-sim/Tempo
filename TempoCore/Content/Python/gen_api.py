...
def generate_tempo_api(root_dir):
    ...
    for tempo_module_name in tempo_module_names:
        ...
        if contains_services:
            ...
            with open(os.path.join(root_dir, output_file), 'w') as f:
                ...
                f.write("from tempo._tempo_context import tempo_context, get_event_loop\n")
    ...
    simple_rpc_template = \
        ...
        "{% endfor %}" \
        ") -> {{ response.full_name }}:\n" \
        "    stub = tempo_context().get_stub({{ service.module_name }}_grpc.{{ service.object_name }}Stub)\n" \
        ...
        ") -> {{ response.full_name }}:\n" \
        "    loop = get_event_loop()\n" \
        "    return loop.run_until_complete(_{{ name }}(\n" \
        "{% for field in request.fields %}" \
        "        {{ field.name }}={{ field.name }}{% if not loop.last %},{% endif %}\n" \
        "{% endfor %}" \
        "    ))\n" \
