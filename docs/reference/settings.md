...
| `bEnableSensorRenderGrouping` | `bool` | `true` | Render the cameras and lidars on one actor that capture at one rate as the views of a single scene render into a shared atlas, paying the per-render fixed costs once per group. Rates cannot change at runtime while enabled (a change is logged and reverted). Read when a sensor activates. See [Render grouping](../plugins/tempo-sensors.md#render-grouping). |
