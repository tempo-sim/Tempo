...
void UTempoSensorServiceSubsystem::RegisterScriptingServices(FTempoScriptingServer* ScriptingServer)
{
	ScriptingServer->RegisterService<SensorService>(
		...
		TStreamingRequestHandler<SensorService, LidarScanRequest, LidarScanSegment>(&SensorService::RequestStreamLidarScans).BindUObject(this, &UTempoSensorServiceSubsystem::StreamLidarScans)
		);
}
