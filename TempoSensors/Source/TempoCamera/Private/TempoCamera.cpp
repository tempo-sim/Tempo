...
{
    // 1. Build the distortion map first
    InitDistortionMap();

    // 2. Call the base class
    Super::InitRenderTarget();
}

// Robust solver: Given the target screen corner (Radius R_target), 
// find the Source Radius (R_source) such that R_target = R_source * (1 + K*R_source^2).
// For Barrel (K<0), R_source will be > R_target (we need a wider image).
// For Pincushion (K>0), R_source will be < R_target (we need a narrower image).
float UTempoCamera::FindRequiredSourceFOV(float TargetDistortedFOV) const
{
    if (TargetDistortedFOV <= 0.0f) return 90.0f;

    // 1. Calculate Target Radius (Tangent of Half FOV)
    // This represents the geometric distance from center to the edge of the screen at Z=1
    float HalfFOVRad = FMath::DegreesToRadians(TargetDistortedFOV / 2.0f);
    double R_target = FMath::Tan(HalfFOVRad);
    
    // 2. Solve for R_source
    // Equation: R_target = R_source * (1 + K1*R_source^2 + K2*R_source^4 + K3*R_source^6)
    // We want to find R_source.
    
    const double k1 = LensParameters.K1;
    const double k2 = LensParameters.K2;
    const double k3 = LensParameters.K3;
    
    // Initial guess: Start with Target. If Barrel, Source is bigger, so maybe multiply by 1.1?
    // Newton-Raphson is fast enough.
    ...
    for (int i = 0; i < max_iter; ++i)
    {
        ...
        // Derivative dF/dR_source
        // d/dR (R + k1*R^3 + k2*R^5 + k3*R^7)
        // = 1 + 3*k1*R^2 + 5*k2*R^4 + 7*k3*R^6
        double Deriv = 1.0 + 3.0*k1*r2 + 5.0*k2*r4 + 7.0*k3*r6;
        
        // Safety clamp for derivative to prevent divide by zero or unstable jumps
    ...
    // Convert R_source back to FOV Degrees
...
#undef UpdateResource
    ...
    for (int v = 0; v < SizeXY.Y; ++v)
    {
        ...
        {
            // Target: The pixel on the output screen
            ...
            // Iteratively solve for the undistorted (source) position 'x, y'
