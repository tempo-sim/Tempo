...
- **Tiled rendering for wide-FOV / fisheye cameras.** Pinhole, Brown-Conrady, and rational-radial models render in a single perspective pass. Kannala-Brandt and Double Sphere fisheye models split into 1, 2, or 4 perspective tiles when the FOV crosses 120° (per axis), with tile rotations and off-axis frustums chosen to evenly distribute pixel density across the field. Up to 240° FOV (Kannala-Brandt) or 280° FOV (Double Sphere). All tiles render in **one** `FSceneViewFamily` so they share scene setup, GPU scene update, Lumen wire-up, and ray-tracing — vs. one `FSceneRenderer` per tile, which costs 2-4× more.
