...
async fn flow_start_stream(state: &mut State) {
    ...
    let needs_window = matches!(
        mt,
        MeasurementType::ColorImage | MeasurementType::DepthImage | MeasurementType::LabelImage | MeasurementType::Video
    );
    ...
    let handle = match (mt, window.clone()) {
        ...
        (MeasurementType::Video, Some(w)) => tokio::spawn(stream_video(sensor, w)),
