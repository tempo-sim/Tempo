...
```sh
python3 Plugins/Tempo/Scripts/migrate_v0_settings_sections.py <YourProjectDir>
```
...
The first v1 build also runs an automatic cleanup of the generated Python tree at `Plugins/Tempo/TempoCore/Content/Python/API/tempo/`. Subdirectories that were generated by v0 but no longer have any protos in v1 (e.g., `TempoCamera/`, `TempoLidar/`, `TempoLabels/`, `TempoSensorsShared/`, `TempoMapQuery/`, `TempoVehicleControl/`, `TempoVehicleMovement/`, `TempoMovementShared/`, `TempoAgentsShared/`, `TempoTime/`, `TempoScripting/`, `TempoCoreShared/`) are removed. You'll see a `[Tempo Prebuild] Removed N stale Python output directories: ...` message in the build log. Custom non-`_pb2` files in any subdir are preserved; the cleanup only removes auto-generated `_pb2.py` / `_pb2_grpc.py` files and empty containers. If you track `Content/Python/API/tempo/` in git, expect a one-time burst of deletions on first v1 build.

### 2. Update your client code

External clients (Python, Rust, C++ wrapper) need to be regenerated against the v1 protos. Regenerate your client and update your imports to the new module names, e.g.:
...
## API naming cleanup (v1)

v1 also normalizes naming across every `.proto`. This is a wire-breaking change for any field, enum value, or message that was renamed — regenerate your client and update the call sites listed below.

### Conventions applied

- **All field names are `snake_case`.** Stragglers in v0 (`Box.Min/Max`, `InstanceSemanticIdPair.InstanceId/SemanticId`) are now `min/max` and `instance_id/semantic_id`.
