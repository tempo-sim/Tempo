...
class TempoMessageDescriptor(TempoObjectDescriptor):
    class FieldDescriptor(object):
        def __init__(self, field_descriptor):
            if field_descriptor.type == 11:
                self.field_type = field_descriptor.message_type.full_name
            elif field_descriptor.type == 14:
...
def generate_tempo_api(root_dir, output_dir):
    all_enums = {}
    all_messages = {}
    all_services = {}
    ...
    for tempo_module_name in tempo_module_names:
        ...
        if contains_services:
            with open(os.path.join(output_dir, f"{pascal_to_snake(tempo_module_name)}.py"), 'w') as f:
    ...
    for service_name, tempo_service_descriptor in all_services.items():
        ...
        with open(os.path.join(output_dir, f"{pascal_to_snake(tempo_module_name)}.py"), 'a') as f:
...
if __name__ == "__main__":
    ...
    generate_tempo_api(root_dir, os.path.join(root_dir))
